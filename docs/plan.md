...
**Status:** Sprints 0-7 shipped (`go build`/`go vet`/`go test` green, Atlas migrations generated), live in production. Patient/OPD/Triage, Consultation/Examination, Laboratory, Pharmacy/Dispensing, Billing/Insurance (ledger, collect/queue endpoints, insurance eligibility/claims, catalog CRUD), and Inpatient (ward/bed/admission/transfer/discharge) are real, working Go code — see "Current State" below and `docs/sprints/` for what shipped in each. hospital-ui has since been built to the same parity (Sprints 1-5 UI, RBAC-gated; Sprint 6 UI in progress) — see `hospital-service/hospital-ui/docs/plan.md`. **2026-08-30:** the User Management Module (auth/roles/permissions) was fully rebuilt — see `docs/architecture.md`'s "User Management Module" section for the complete writeup; this is new capability beyond the original Sprint 0-13 roadmap below, not part of any numbered sprint.
...
A fourth research pass (2026-09-02) closed a real gap a client-facing engineer flagged: IPD/OPD referral, transfer, and ambulance workflows were documented too thinly to actually reflect real hospital practice, even though the design direction already pointed the right way. This pass researched Kenya's real national referral guideline and its documented adherence problems, how OpenMRS/KenyaEMR itself models a ward/bed transfer as a distinct ADT event, and how Kenyan private ambulance operators price and separate a standalone call-out from an inter-facility patient transfer, then turned that research into additive field designs: richer inter-facility fields on `Referral`, a new `PatientTransfer` entity, and a real billing linkage from `AmbulanceBooking` into `PatientAccount`/`BillableCharge`. All of it is planned design, not yet built. Full detail: `docs/architecture.md`'s "Referral, Transfer & Ambulance Billing" section, `docs/erd.md`, `docs/integrations.md` §2A.1/§2D.1, and the relevant `docs/sprints/` files.
...
**2026-09-02 — chemist/pharmacy workflow gap-fix pass** (prompted by a code audit that traced every gap between the documented design and the real shipped code): fixed a real chemist-tier data gap where a walk-in dispense never posted a billable charge at all (new `WalkInSale` entity, the ledgerless counterpart to `PatientAccount`/`BillableCharge`); wired the previously dead `SignSaleNow`/`hospital_sale` eTIMS attribution end to end (required a small treasury-api fix too, since its S2S sign-pos-sale endpoint silently dropped any `source` field); fixed four frontend/backend RBAC permission mismatches and added a missing Lab Technician role (no seeded role but Admin could enter lab results before this); added a real drug catalog search to prescription creation; wired the previously-unused insurance eligibility-check/claim-poll hooks into the UI; closed the `Referral.status`/`PatientVisit.cancelled` dead-code paths with real Cancel actions; resurfaced `lab_complete` visits in the consultation queue; and removed one confirmed-dead enum value (`LabOrder.status`'s `collected`). Full detail: `.claude/plans/hospital-chemist-pharmacy-remediation-2026-09-02.md` and `.claude/memory/hospital-chemist-pharmacy-workflow-audit-2026-09-02.md`.

**2026-09-02 — Sprint 6 (Inpatient) shipped**: `Ward`/`Bed`/`Admission`/`PatientTransfer` ent schemas + Atlas migration (4 new tables, zero changes to existing ones); `internal/modules/inpatient` (Ward/Bed CRUD, `GetWardOccupancy`, `Admit`, `Transfer` — both intra-facility bed/ward moves and inter-facility transfer-out, `Discharge`); `billing.Service.PostCharge` is now admission-aware (a visit with an active admission routes every department's charge onto the admission's own account, transparently — zero changes needed in triage/consultation/lab/pharmacy); discharge posts the ward/day-rate charge segmented by ward-occupied-when (via `PatientTransfer` history) and gates on a zero balance (409 + `ErrOutstandingBalance`, same Record-Payment/Apply-Insurance/Write-Off/next-of-kin escape hatches Sprint 5 already built); RBAC extended (`PermInpatientAdd` to Doctor/Nurse/Manager, `PermInpatientManage` to Doctor/Manager). New `internal/integration/inpatient_golden_path_test.go` walks the full admit→charge-while-admitted→transfer→blocked-discharge→settle→discharge chain against real service-layer code. A `PatientTransfer` entity and the richer inter-facility referral/transfer/ambulance-billing design were added to this sprint's scope mid-session after a docs research pass found the original plan had no transfer concept at all — see `docs/architecture.md`'s "Referral, Transfer & Ambulance Billing" section. Full detail: `.claude/plans/hospital-sprint6-inpatient-2026-09-02.md`.

**2026-09-02 — Sprint 7 (Theatre/OT + ICU) shipped, same session**: `TheatreBooking`/`ICUEpisode` ent schemas + Atlas migration (2 new tables, zero changes to existing ones); `internal/modules/theatre` (booking creation with room/time-slot conflict detection, a prepayment gate mirroring lab's own `requires_prepayment` pattern exactly, checklist, start/complete/cancel lifecycle) and `internal/modules/icu` (episode start/update/end, one active episode per admission at a time); a theatre booking's procedure fee posts via the SAME admission-aware `billing.PostCharge` Sprint 6 introduced — a booking for an admitted patient's visit lands on the admission's account automatically, zero new billing code needed. ICU's elevated day-rate is NOT modeled as a separate billing path — an ICU bed's Ward carries its own `billable_item_code` (Sprint 6), so discharge-time billing already charges correctly. New RBAC permission set (`PermICU*`) added; role grants split by real clinical ownership (Doctor runs theatre end to end + observes ICU, Nurse runs ICU end to end + views theatre only, Manager gets both broadly). New `internal/integration/theatre_icu_golden_path_test.go`.

**Not yet implemented**: hospital-ui's Sprint 6/7 pages, and Sprints 8-13 (Blood Bank onward — Sprint 9's ambulance-billing linkage design is ready, tracked as Phase 8 of `hospital-sprint6-inpatient-2026-09-02.md`, not yet built). Per-user outlet/branch membership enforcement remains a known gap (`internal/http/middleware/outlet_context.go` — any authenticated tenant user can select any outlet via `X-Outlet-ID` today). See the phased roadmap below and `docs/sprints/`.
...
| 4 | Pharmacy & Dispensing — migrated from pos-api's `Prescription`/`PrescriptionLine`/`ControlledSubstanceLog`; calls `inventory-api` for drug master/lot/interactions; includes the standalone-chemist module-toggle configuration (see `docs/migration-pos-pharmacy.md` § 6) | ✅ Core dispensing lifecycle shipped 2026-08-29 (`hospital-api@4005c21`); dispensing-label PDF endpoint shipped 2026-08-30; a real drug catalog search (`inventory.Client.SearchItems` + `GET /pharmacy/drug-search`, replacing free-text-only entry) shipped 2026-09-02 |
| 5 | Billing & Insurance — calls `treasury-api` for invoices + SHA/SHIF/NHIF eligibility/claims (eTIMS opt-in per tenant/service) | ✅ Core ledger shipped 2026-08-29 (`hospital-api@126adbf`) — charge/collect/queue/settle/override-settlement; insurance eligibility/claim endpoints wired into real checkout flows 2026-08-30, plus an eligibility-check/status-poll UI 2026-09-02; Chemist-tier `WalkInSale` (ledgerless checkout) and eTIMS `hospital_sale` sign-at-collect both shipped 2026-09-02 — see the audit/remediation plan for full detail |
| 6 | Inpatient — Ward/Bed/Admission, transfer, discharge summaries | ✅ Shipped 2026-09-02 (backend) — `hospital-ui` pages in progress |
| 7 | Theatre/OT scheduling + ICU/Critical-care monitoring | ✅ Shipped 2026-09-02 (backend) — `hospital-ui` pages in progress |
...
3. **Consultation** — doctor/dental/MCH/specialist queues, structured examination notes, diagnosis capture, referral to lab/pharmacy (an internal, same-visit hand-off) or to another facility entirely (a genuine inter-facility referral, with a referral letter, receiving-facility identity, and counter-referral feedback — planned, additive fields on `Referral`, see `docs/erd.md` and `docs/architecture.md`'s "Referral, Transfer & Ambulance Billing" section, 2026-09-02).
...
6. **Inpatient** — ward/bed assignment, admission-to-discharge, discharge summaries, and ward-to-ward/bed-to-bed **transfer** during an ongoing admission (a differential day-rate applies from the transfer date forward) plus **inter-facility transfer** of an active inpatient to another facility (distinct from an OPD referral — the admission closes, usually with an ambulance leg). Planned, additive design: `docs/erd.md`'s new `patient_transfer` table and `docs/sprints/sprint-6-inpatient.md`, 2026-09-02.
...
10. **Ambulance & Emergency Dispatch** — thin reference into logistics-api's existing Task (`task_type: ambulance_dispatch`, an additive string value, no schema change)/FleetMember (tagged `ambulance`)/PricingRule (`distance` rule type, matching Kenya's base-fee-plus-per-km ambulance pricing model) — hospital-api does not build a second dispatch/fleet engine. Optional recurring "ambulance membership" product (mirrors St John Kenya's individual/family annual plan) billed via treasury-api. **Billing linkage and transfer/referral-triggered dispatch (planned, 2026-09-02):** a completed booking's fare now posts as a `BillableCharge` onto the patient's own `PatientAccount` when one is already open (an admitted inpatient's inter-facility transfer, or an OPD patient with an open account), or stays a standalone/no-ledger charge for a call-out with no prior registration — see `docs/architecture.md`'s "Referral, Transfer & Ambulance Billing" section and `docs/integrations.md` §2A.1.
11. **Asset / Equipment integration** — surfaces inventory-api's existing `Asset`/`AssetMaintenance` register (already covers biomedical equipment, beds, ambulances-as-capital-assets, warranty, maintenance schedules) as "Biomedical Equipment" in the hospital-api UI; hospital-api references equipment via `Bed`/`TheatreBooking`/`ICUEpisode.equipment_asset_ids`, never owns a parallel asset register — **shipped 2026-09-02**, brought forward from Sprint 9, see `docs/architecture.md`'s "Biomedical Equipment / Asset Integration" section. Depreciation accounting is already wired via treasury-api's `FixedAssetDepreciation`.

// Package sequence allocates human-readable, per-tenant monotonic numbers (mrn, visit_number)
// backed by the DocumentSequence table. Allocation uses SELECT ... FOR UPDATE inside the
// caller's transaction so concurrent registrations/check-ins never collide. Numbers are
// rendered from a configurable template and the counter can reset yearly/monthly. Ported from
// library-api's internal/modules/sequence (which itself adapted treasury-api's original
// document-sequence pattern) rather than reinvented, per sprint-1-patient-opd-triage.md.
package sequence
...
// Next allocates and returns the next formatted number for (tenant, kind) within tx, creating
// the counter row on first use with kind-appropriate defaults. Applies the configured reset
// period (counter restarts at 1 on a new year/month) and renders the number from the configured
// format template.

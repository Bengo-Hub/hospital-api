...
type Prescription struct {
	...
	// The consultation this prescription was written from, when there is one
	ExaminationID *uuid.UUID `json:"examination_id,omitempty"`
	// Set when dispensing against a prescription written elsewhere (chemist walk-in)
	...
	Status prescription.Status `json:"status,omitempty"`
	...
	// allergy_flags, interaction_check_id, approved_by/at, reservation_id, cancel_reason, approval_override_reason
...
func (e PrescriptionEdges) LinesOrErr() ([]*PrescriptionLine, error) {
	if e.loadedTypes[0] {
		return e.Lines, nil
	}
...
func (*Prescription) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case prescription.FieldMetadata:
			values[i] = new([]byte)
		case prescription.FieldExternalFacilityName, prescription.FieldPrescriptionNumber, prescription.FieldPrescriberName, prescription.FieldPrescriberLicense, prescription.FieldPatientName, prescription.FieldPatientIDNumber, prescription.FieldStatus, prescription.FieldNotes:
			values[i] = new(sql.NullString)
		case prescription.FieldPatientDob, prescription.FieldDispensedAt, prescription.FieldCreatedAt, prescription.FieldUpdatedAt:
	...
	for i := range columns {
		...
		case prescription.FieldPatientDob:
			if value, ok := values[i].(*sql.NullTime); !ok {
				...
				_m.PatientDob = new(time.Time)
				*_m.PatientDob = value.Time
			}
		...
		case prescription.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Status = prescription.Status(value.String)
			}

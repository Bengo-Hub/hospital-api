...
// This is the ONLY tenant-scoped RBAC table — it ties a (global) HospitalRole to a
// HospitalUser within a specific tenant.
...
func (UserRoleAssignment) Fields() []ent.Field {
	return []ent.Field{
		...
		field.UUID("user_id", uuid.UUID{}).
			Comment("User identifier (hospital_user)"),
...
func (UserRoleAssignment) Edges() []ent.Edge {
	return []ent.Edge{
		edge.To("user", HospitalUser.Type).
			Field("user_id").
			Required().
			Unique(),
		edge.To("role", HospitalRole.Type).
			Field("role_id").
			Required().
			Unique(),
	}

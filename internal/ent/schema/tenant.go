...
//
// Auth-api is the single source of truth for tenant identity, branding, and subscription
// data. hospital-api stores ONLY the minimal reference needed for FK relationships and
// routing — per the tenant_data_architecture platform rule, branding/logo/color/contact
// fields never live here; they are fetched from auth-api on demand or read from JWT claims.
type Tenant struct {
	ent.Schema
}
...
func (Tenant) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("use_case").
			Optional().
			Nillable().
			Comment("Primary business use case — synced from auth-api (e.g. hospital)"),
...
func (Tenant) Edges() []ent.Edge {
	return []ent.Edge{
		edge.To("users", HospitalUser.Type),

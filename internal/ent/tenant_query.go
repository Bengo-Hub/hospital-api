...
func (_q *TenantQuery) QueryUsers() *HospitalUserQuery {
...
func (_q *TenantQuery) WithUsers(opts ...func(*HospitalUserQuery)) *TenantQuery {
	query := (&HospitalUserClient{config: _q.config}).Query()
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withUsers; query != nil {
		if err := _q.loadUsers(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Users = []*HospitalUser{} },
			func(n *Tenant, e *HospitalUser) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TenantQuery) loadUsers(ctx context.Context, query *HospitalUserQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *HospitalUser)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(hospitaluser.FieldTenantID)
	}
	query.Where(predicate.HospitalUser(func(s *sql.Selector) {
